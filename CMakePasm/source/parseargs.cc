...
void usage(void)
{
    ...
    fprintf(console, "  pasm [-o outputfile] [-d sym val] [-dir directory] [-s symbolfile] [-l listfile] [-log logfile] [-i] [-v] [-c64] [-65c02 | -6502] [-?] [-nowarn] [-vers] inputfile1 inputfile2 ...\n\n");
    ...
    fprintf(console, "         -vers           display version number\n");
...
int parse_arguments(const int argc, char* argv[])
{
    ...
    while (arg_index < argc) {
        ...
        // version
        if (stricmp(argv[arg_index], "-vers") == 0 || stricmp(argv[arg_index], "/vers") == 0) {
            arg_index++;
            vers_specified = true;
            fprintf(console, "pasm version %s\n", VERSION);
            fprintf(console, "built on %s %s\n\n", __DATE__, __TIME__);
            continue;
        }
